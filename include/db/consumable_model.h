...
const long PUSH_EXPECTED_DURATION = 35 * 60 * 60;
...
struct Consumable {
    ...
    Consumable(long sweepExpected, long mopExpected, long vacuumExpected, long pushExpected, long aromatherapyExpected,
               ...
               long disinfectUse) : sweep_expected(sweepExpected), mop_expected(mopExpected),
                                    vacuum_expected(vacuumExpected), push_expected(pushExpected),
                                    aromatherapy_expected(aromatherapyExpected), disinfect_expected(disinfectExpected),
                                    sweep_use(sweepUse), mop_use(mopUse), vacuum_use(vacuumUse), push_use(pushUse),
