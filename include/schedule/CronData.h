...
namespace croncpp {
    ...
    public:
        ...
        static const croncpp::Months months_with_31[NUMBER_OF_LONG_MONTHS];
    ...
    std::string &CronData::replace_string_name_with_numeric(std::string &s) {
        ...
        static_assert(std::is_same<T, croncpp::Months>()
                      || std::is_same<T, croncpp::DayOfWeek>(),
                      "T must be either Months or DayOfWeek");

        if constexpr (std::is_same<T, croncpp::Months>()) {
