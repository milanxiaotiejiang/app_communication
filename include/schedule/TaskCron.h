...
namespace croncpp {
    ...
    public:
        ...
        TaskCron(std::string name, std::string sch, std::string end_sch, bool neverend, int endYear,
                 const CronSchedule schedule, const CronSchedule end_schedule, TaskFunction task)
                : name(std::move(name)), s_schedule(std::move(sch)), s_endschedule(std::move(end_sch)),
                  never_end(neverend), end_year(endYear), schedule(std::move(schedule)),
                  end_schedule(std::move(end_schedule)), task(std::move(task)) {
        }
