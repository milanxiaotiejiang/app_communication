...
namespace cppfs {
    ...
    FileWatcher::FileWatcher(FileWatcher &&fileWatcher)
            : m_backend(std::move(fileWatcher.m_backend)), m_eventHandlers(std::move(fileWatcher.m_eventHandlers)),
    ...
    void FileWatcher::addHandler(FileEventHandler *eventHandler) {
        ...
        if (!eventHandler ||
            std::find(m_eventHandlers.begin(), m_eventHandlers.end(), eventHandler) != m_eventHandlers.end()) {
            return;
        }
    ...
    void FileWatcher::addHandler(EventFunc funcFileEvent) {
        ...
        auto ptr = std::unique_ptr<FunctionalFileEventHandler>(
                new FunctionalFileEventHandler(std::move(funcFileEvent)));
