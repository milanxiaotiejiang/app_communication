...
namespace cppfs {
    ...
    Tree::Tree()
            : m_directory(false), m_size(0), m_accessTime(0), m_modificationTime(0), m_userId(0), m_groupId(0),
              m_permissions(0) {
    }
    ...
    void Tree::createDiff(const Tree *currentState, const Tree *targetState, Diff &diff) {
        ...
        for (const auto &targetFile: targetState->children()) {
            ...
            Tree *currentFile = (it != currentFiles.end()) ? currentState->children()[std::distance(
                    currentFiles.begin(), it)].get() : nullptr;
