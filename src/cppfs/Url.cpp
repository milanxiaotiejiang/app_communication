...
namespace cppfs {
    ...
    Url::Url()
            : m_url(""), m_analyzed(false), m_protocol(""), m_location(""), m_address(""), m_path(""), m_login(""),
              m_host(""), m_username(""), m_password("") {
    }

    Url::Url(const Url &url)
            : m_url(url.m_url), m_analyzed(url.m_analyzed), m_protocol(url.m_protocol), m_location(url.m_location),
              m_address(url.m_address), m_path(url.m_path), m_login(url.m_login), m_host(url.m_host),
              m_username(url.m_username), m_password(url.m_password) {
    }

    Url::Url(Url &&url)
            : m_url(std::move(url.m_url)), m_analyzed(std::move(url.m_analyzed)), m_protocol(std::move(url.m_protocol)),
              m_location(std::move(url.m_location)), m_address(std::move(url.m_address)), m_path(std::move(url.m_path)),
              m_login(std::move(url.m_login)), m_host(std::move(url.m_host)), m_username(std::move(url.m_username)),
              m_password(std::move(url.m_password)) {
    }

    Url::Url(const std::string &url)
            : m_url(url), m_analyzed(false), m_protocol(""), m_location(""), m_address(""), m_path(""), m_login(""),
              m_host(""), m_username(""), m_password("") {
    }

    Url::Url(std::string &&url)
            : m_url(std::move(url)), m_analyzed(false), m_protocol(""), m_location(""), m_address(""), m_path(""),
              m_login(""), m_host(""), m_username(""), m_password("") {
    }
